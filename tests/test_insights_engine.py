...
class BuildInsightsRealDataTests(unittest.TestCase):
    ...
    def test_alerta_precio_fuera_de_mercado_en_carulla(self):
        matches = self._find("alertas", "precio_fuera_de_mercado", "Carulla")
        self.assertEqual(len(matches), 1, "Se esperaba exactamente una alerta de precio para Carulla")
        insight = matches[0]
        self.assertAlmostEqual(insight["valor_actual"], 188.5, delta=0.5)
        self.assertIn("188", insight["mensaje_especifico"])
