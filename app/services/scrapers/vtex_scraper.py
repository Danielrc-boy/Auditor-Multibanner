...
async def run_vtex_scraping(conn) -> int:
    ...
    # Se importa directamente dentro del scope de la función para evitar la importación circular
    ...
    for retailer_name in ["exito", "carulla"]:
        ...
        for term in search_configs:
            try:
                ...
                if results:
                    count = main_module.save_scraper_results(conn, results, retailer=retailer_name)
                    total_saved += count
