...
class VTEXScraper:
    ...
    async def search_keyword(self, keyword: str, limit: int = 50) -> List[ExtractedProductData]:
        # Formatear término de búsqueda de forma segura
        clean_keyword = urllib.parse.quote(keyword.strip())
        catalog_url = f"{self.base_url}/api/catalog_system/pub/products/search/{clean_keyword}?_from=0&_to={limit-1}"
        
        req_url, params = self._build_request(catalog_url)
        ...
        async with httpx.AsyncClient(timeout=25.0, verify=False, follow_redirects=True) as client:
            try:
                ...
                print(f"[WARN {self.retailer.upper()}] Catalog API falló: {e}", flush=True)
